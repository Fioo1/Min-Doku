...
const fallback = { name: 'Jugador', level: 1, xp: 0, coins: 100, streak: 0, best_streak: 0, streak_shields: 2, last_played: null, solved: 0, best_time_seconds: null, bestTime: '--:--', games_played: 0, wins: 0, total_mistakes: 0, hints_used: 0 }
...
export function AuthProvider({ children }) {
  ...
  const updateProfile = async patch => { const next = normalize({ ...profile, ...patch }); setProfile(next); if (!user) return next; const record = {
      id: user.id,
      name: next.name,
      ...
      last_played: next.last_played,

      solved: next.solved,
      best_time_seconds: next.best_time_seconds,
      games_played: next.games_played,
      wins: next.wins,
      total_mistakes: next.total_mistakes,
